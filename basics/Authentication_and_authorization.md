...
#### *A. Sessions*
*   *The Problem:* HTTP is fundamentally a stateless protocol, meaning it remembers nothing about past requests. Modern web apps (like e-commerce sites) need stateful memory, if user wants to access multiple different pages then he must be authenticated once entered.
*   *The Solution:* The server creates a unique *Session ID* upon login, storing the user's data alongside this ID in a persistent, fast in-memory(RAM) store like Redis. The Session ID is sent to the client as cookie and included in all future requests, giving the server memory.
                     this session has expiration timing after which the data is automatically deleted. 
...
*   *The Solution:* JWTs are a *stateless, self-contained* mechanism that offloads storage from the server / transferring claims between different parts or system stateless.
*   *Structure:* They contain three base64-encoded parts:
    1.  *Header:* Metadata like the signing algorithm.
    2.  *Payload:* data of the user that we store in jwt, Contains "claims" such as the user ID (`sub`), issued at time (`iat`), and roles.
    3.  *Signature:* Verified using a secret key held only by the server to ensure the token hasn't been tampered with.
    
*  *Benefits of using JWT*
   1. *Stateless* we don't have to store the session data anymore, session verification is now stateless
   2. *Scalable*  a cookie isused from one server cannot be used by other server in microservice architecture so JWT enables client to send jwt to different servers which will have secret key using that jwt can be decoded
   3. *Portable* 
* Token Theft (ISSUE) any person having the JWT Token can access the data 
